...
public class PoolDB {
    ...
    public static Connection getConnection(String nName)
        ...
        ds = (DataSource) envContext.lookup((new StringBuilder()).append("Pool").append(nName).toString());
        if (ds == null) {//Si no se encuantra Buscar desde el contexto JDBC
            ds = (DataSource) cxt.lookup((new StringBuilder()).append("jdbc/Pool").append(nName).toString());
        }
        if (ds == null) {
